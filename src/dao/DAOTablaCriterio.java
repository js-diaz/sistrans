...
public class DAOTablaCriterio {
	 ...
	 */
	public List<ContenedoraInformacion> generarListaFiltradaZonas(List<Criterio> criteriosOrganizacion,
			List<Criterio> criteriosAgrupamiento,List<Criterio> agregacionesSeleccion, 
			CriterioVerdad operacionesWhere, CriterioVerdad operacionesHaving) throws SQLException, Exception {
		...
		List<Criterio> agreSelec=new ArrayList<>();
		...
		for(Criterio a: agregacionesSeleccion)
		{
		...
		if(operacionesHaving!=null)
			{
				for(Criterio c:operacionesHaving.getCriterios())
				{
					operaciones=simplificar(c.getNombre());
					if(operaciones.trim().equals("")) continue;
					if(buscarCriteriosZona(operaciones)==null)
	...
	private void evaluarWhereZona(String nombre) throws SQLException, Exception{
		String temp=nombre.replaceAll("\\(","").replaceAll("\\)","").trim();
		for(CriterioVerdad.PalabrasVerdad p:CriterioVerdad.PalabrasVerdad.values())
		{
			temp=temp.replaceAll(p+"","");
		...
		{
			if(s.trim().equals("")) continue;
			if(buscarCriteriosZona(s.trim())==null) throw new Exception("Hay un criterio del where que no existe "+s);
		}
	}
	...
	private String simplificarAgrupacion(String nombre) {
		...
		if(nombre.contains("\\*")) temp=temp.replaceAll("\\*","");
		return temp.trim();
	}
